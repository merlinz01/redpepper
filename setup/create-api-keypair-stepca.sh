...
*/30 * * * *   root   STEPPATH=/etc/redpepper-step-ca/ step ca renew --force --expires-in 24h /etc/redpepper/api-cert.pem /etc/redpepper/api-key.pem --exec "systemctl restart redpepper-manager > /var/log/redpepper/api-renew.log 2>&1"
EOF
